...
public class AutoGenerator {
    ...
    private Map<String, VelocityContext> analyzeData(ConfigBuilder config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            /* ---------- 添加导入包 ---------- */
            if (StringUtils.isEmpty(config.getSuperEntityClass()) && config.getGlobalConfig().isActiveRecord()) {
                // 开启 ActiveRecord 模式
            ...
            ctx.put("model", config.getPackageInfo().get(ConstVal.MODEL));            
            //若未配置业务数据模型名称，则采用实体名+BO的形式
            String modelName = getGlobalConfig().getModelName();
            if(null == modelName || "".equals(modelName.trim())){
              modelName = tableInfo.getEntityName()+"BO";
            } 
            ctx.put("modelName", modelName);
            
            String uriPath = config.getStrategyConfig().getUriPath();
            if(StringUtils.isNotEmpty(uriPath)){
            	ctx.put("uriPath", uriPath);
                ctx.put("permissionPath", uriPath.substring(1).replaceAll("/", ":"));
            }
    ...
    private void batchOutput(String entityName, VelocityContext context) {
        try {
            ...
            String modelFile = String.format((pathInfo.get(ConstVal.MODEL_PATH) + File.separator + modelName + ConstVal.JAVA_SUFFIX), modelName);

            TemplateConfig template = config.getTemplate();

            // 根据override标识来判断是否需要创建文件
            if (isCreate(entityFile) && config.getGlobalConfig().isOpenPersistence()) {
                vmToFile(context, template.getEntity(), entityFile);
            }
            if (isCreate(mapperFile) && config.getGlobalConfig().isOpenPersistence()) {
                vmToFile(context, template.getMapper(), mapperFile);
            }
            if (isCreate(xmlFile) && config.getGlobalConfig().isOpenPersistence()) {
                vmToFile(context, template.getXml(), xmlFile);
            }
            if (isCreate(serviceFile) && config.getGlobalConfig().isOpenService()) {
                vmToFile(context, template.getService(), serviceFile);
            }
            if (isCreate(implFile) && config.getGlobalConfig().isOpenService()) {
                vmToFile(context, template.getServiceImpl(), implFile);
            }
            if (isCreate(controllerFile) && config.getGlobalConfig().isOpenController()) {
                vmToFile(context, template.getController(), controllerFile);
            }
            if(isCreate(modelFile) && config.getGlobalConfig().isOpenModel()) {
                //输出业务model
                vmToFile(context, template.getModel(), modelFile);
            }
